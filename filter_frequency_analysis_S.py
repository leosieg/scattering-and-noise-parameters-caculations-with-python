...
#
# frequency babd
freq = np.linspace(0.5, 1.5, 1001)*1e9
dsp = 0
#
#
#start = ti.time()
[s11, s12, s21, s22] = cir.filter_circuit_S(freq, dsp)
#ende = ti.time()
#delta_t = ende - start
#print(delta_t)
#
...
plt.axis('equal') 
#
plt.plot(np.real(s11[0,:]),np.imag(s11[0,:]), lw = 1.5, color= 'black')
#
plt.figure(4)#########################################
#
...
plt.axis('equal') 
#
plt.plot(np.real(s22[0,:]),np.imag(s22[0,:]), lw = 1.5, color= 'black')
#
plt.figure(6)######################################### 
#
